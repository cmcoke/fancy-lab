...
/**
 * -------------------------------------------------------------
 * WooCommerce Layout & Hook Customization
 * -------------------------------------------------------------
 *
 * Purpose:
 * Modifies the default WooCommerce page structure using
 * actions and filters to implement a custom Bootstrap-based layout.
 *
 * What this function does:
 * - Wraps WooCommerce content inside custom container and row markup
 * - Repositions the default WooCommerce sidebar
 * - Applies a sidebar layout ONLY on the main Shop archive page
 * - Uses conditional logic (is_shop()) to control column widths
 * - Outputs product excerpts in the shop loop
 * - Optionally allows modification of the shop page title (filter)
 *
 * Layout Strategy:
 * - Shop page: Sidebar (3 columns) + Content (9 columns)
 * - Other WooCommerce pages: Full-width content
 *
 * Technical Notes:
 * - Uses add_action() and remove_action() to override
 *   WooCommerce default hook behavior.
 * - Uses hook priorities to control layout order.
 * - Runs on the 'wp' action to ensure conditional tags
 *   like is_shop() work correctly.
 *
 * In short:
 * This function centralizes all WooCommerce structural
 * modifications for the theme in one controlled location.
 * -------------------------------------------------------------
 */
...
function fancy_lab_wc_modify()
{
  ...
  /**
   * Apply custom sidebar layout only on the WooCommerce Shop archive.
   *
   * is_shop() returns true only for the main Shop page.
   * This prevents the sidebar structure from affecting:
  ...
  function fancy_lab_add_shop_tags()
  {
     ...
     *
     * If on the main Shop archive page:
     * - Use a reduced-width column to accommodate the sidebar.
     *
     * Otherwise:
     * - Use a full-width column (no sidebar layout).
     */
